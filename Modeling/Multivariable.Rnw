...
It may seem odd to your students that a word (or \newword{character string}) can be used as the input to a function.  Remind them that a function is a machine that takes one or more inputs and produces an output.  There's no requirement that the inputs be numbers.
...
The function \function{g2} takes just a single character string.  To allow it to work on a vector of character strings, you can \newword{vectorize} it:\AuthNote{Any way to avoid this?}
<<>>=
g2 = Vectorize(g2)
@
...
A fitted model \newword{partitions} variation between that accounted for by the model and that which remains unaccounted for.  Measure the variation accounted for using the variation in the fitted model values; measure the rest of the variation using the residuals.  For instance, for \texttt{mod1}:
<<>>=
sd(fitted(mod1))
sd(resid(mod2))
@
...
You can also add in \newword{interactions} among explanatory variables.
...
Including such a difference in a model is a matter of including an \newword{interaction term} between \VN{sex} and \VN{year}:
<<>>=
