...
The R system provides a way for the software development community to add new functionality.  
``Packages" are the standard way to deliver such software.  
The statement \code{require(mosaic)} loads in the \pkg{mosaic} package, which itself 
includes several other packages.  

The \pkg{mosaic} package provides many of the commands that are used in this book.
We will also make use the \pkg{mosaicData} package which contains a number of data sets
used in examples.
...
\marginnote{%
The \dataframe{trebuchet} data set was collected by a high-school student, Andrew Pruim, 
as part of a Science Olympiad competition.  These data are included in the \pkg{fastR} package.
}
...
As always in these notes, you'll be using, among other things, 
functions from the \pkg{mosaic} package and data sets from \pkg{mosaicData}.  
When working with your own data, the \pkg{mosaicData} package is not required.
You need to load this into R:
<<>>=
require(mosaic)
require(mosaicData)
@
This will also load allied packages, such as \pkg{lattice} graphics.  Often, you will encounter methods or data that you want to use in others of the many packages available for R.  For instance, the next example involves a data set about a toy trebuchet available in the \pkg{fastR} package:
<<>>=
require(fastR)
@

\authNote{I'm simplifying the trebuchet data a bit, considering only \code{form=='a'}.}
<<>>=
...
\authNote{What is the point of this marginal note?}
\marginnote{It's tempting to show students shortcuts for looking at a data set, 
such as \function{summary}.}
<<eval=FALSE>>=
summary(trebuchet)
@
...
To introduce students to a notation that makes such relationships explicit, \pkg{mosaic} provides \function{makeFun}:\marginnote{Math students get used to the convention of writing functions as $f(x)$ or $g(x)$.  You'll have to take care to remind them that $f$ is the function and $x$ is the name of the input. In these notes, we tend to use names like \variable{f1} and \variable{f2} for functions, and \variable{mod1} and \variable{mod2} for the statistical models.}
<<>>=
f = makeFun(4*x + 2 ~ x)
@
...
The model, \variable{mod1}, created earlier, is not exactly a function.\marginnote{You can see what's in \variable{mod1} by giving it as a command, 
<<eval=FALSE>>=
mod1
@
This displays the model coefficients.  In addition to \function{makeFun}, you can use such operations as \function{rsquared}, \function{fitted}, and \function{resid}.} It's set up this way because there are several different types of information one might want to get from a model, not just the model function.  You can use \function{makeFun} to extract the model function from \variable{mod1}: 
<<>>=
f1 = makeFun(mod1)
@
This use of \function{makeFun} reformats the information that's already in \variable{mod1}  as a mathematical function in R. Once that's done, the function can be evaluated in the ordinary way, by specifying the inputs.

The function \variable{f1} attempts to describe the relationship between the explanatory and response variables: between \variable{distance} and \variable{projectileWt}.


There are, of course, other possible functions that could be used to describe the relationship between \variable{distance} and \variable{projectileWt}.  Taking some measurements off the scatter plot, one might reasonably try a simple function, perhaps $y = 1900 - 30*x$.  No reason to use the names $y$ and $x$, however.  You can define the proposed function using the names in the data set:
<<>>=
...
There are several ways to see the effect of \variable{projectileWt} on \variable{distance}.  The coefficients carry this information, but students need to be taught how to interpret them:
<<>>=
coef(mod1)
@
This says that for every 1 gm increase in the weight of the projectile, the predicted distance flown decreases by 14.3 cm.  (The units of the variables are given in the help file for the data, \code{help(trebuchet)}.)
...
The first calculates the groupwise means of \variable{distance}, with the groups defined by \variable{object}.
<<>>=
vals = mean(distance~object,data=trebuchet)
vals
@

The second statement fits a model that accounts for the variation in \variable{distance} by the variation in \variable{object}:
...
\noindent As it happens, the model values from \variable{mod} coincide directly with the values produced by \function{mean}. To see this, extract the function from the model:
...
The function \function{g1} can be used to calculate a model value for each case in the \dataframe{trebuchet} data and, from that, the residuals:
...
@
\end{widestuff}
This is not the traditional form for a function.  The function \variable{g2} takes an input \variable{object} and compares it to the names of the different kinds of balls.  It returns the associated value (or \variable{NA} if there is no associated value), for instance:
...
Earlier, the standard deviation was used as a way to quantify the size of residuals.  Let's use it now to quantify the size of variation in the response variable.  For the \variable{distance} variable in the \dataframe{trebuchet} data, this is:
<<>>=
sd( distance,data=trebuchet)
@
Now consider the size of the variation in the model values from the various models we have fitted --- \variable{mod1}, \variable{mod2}, and \variable{mod3} from which were extracted the model functions \function{f1}, \function{ballistic}, and \function{g2}:
<<>>=
...
A fitted model \term{partitions} variation between that accounted for by the model and that which remains unaccounted for.  Measure the variation accounted for using the variation in the fitted model values; measure the rest of the variation using the residuals.  For instance, for \variable{mod1}:
<<>>=
sd(fitted(mod1))
sd(resid(mod2))
@
...
An important question is whether $R^2$ can be used to compare different model designs to decide which is best.  For instance, $R^2$ from the groupwise-mean model \variable{mod3} is somewhat larger than for \variable{mod1}:
<<>>=
rsquared(mod3)
@
Does this mean that \variable{mod3} is better than \variable{mod1}?  That will turn out to be a productive route to studying statistical inference.  But before heading in that direction, let's expand the set of models that students can build and interpret.
...
\paragraph{Three}. Show that \variable{object} gives a perfect prediction of \variable{projectileWt} in the \dataframe{trebuchet} data. Does \variable{object} contribute anything beyond \variable{projectileWt} in explaining \variable{distance}?
