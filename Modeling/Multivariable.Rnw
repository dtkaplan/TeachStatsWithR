...
See the mosaic resampling vignette \cite{resampling} for more information.}
...
<<tidy=FALSE>>=
g2 = makeFun(
  switch(object,"golf"=300,"foose"=500,"tennis ball"=100,"big washerb"=300,NA) ~ 
    object)
@
\end{figure*}
...
Will writes:
\begin{quotation}
The fact that the quality of schools correlates more positively with the quality of the families from which children come to school than it does with education appropriatiations will have no effect on the teachers unions' insistence that money is the crucial variable.
\end{quotation}
...
<<>>=
tally( ~result >= test.stat, data=s, format="proportion")
@
...
tally( ~result >= test.stat2, data=s2, format="proportion")
@
