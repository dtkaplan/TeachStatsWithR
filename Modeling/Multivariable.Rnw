...
These notes are about a strategy for teaching statistical modeling.  The strategy involves treating modeling with multiple variables as a notation extension of relatively simple concepts such as groupwise means.  In order to prepare students for the extension, it helps to present groupwise means as a sort of model, even though the traditional way is algorithmically: an average for the members in each group.  The motivation for the strategy can be understood metaphorically: by not leading students into a conceptual dead end, there is no need to retrace the path to get to the actual destination.
...
It would be possible to implement the strategy presented here using other software.  Indeed, it would be possible to present it without using software at all.  But the R computer notation is simple and concise, a rival for traditional mathematical notation.  The software is freely available.  So even if you don't use R, we hope you can learn the strategy by presenting it in R notation.
...
\marginnote{The statement \texttt{require(mosaic)} loads in the \pkg{mosaic} package, which itself includes the graphics package \pkg{lattice}.  The \pkg{mosaic} provides many of the commands that are used in this book.}

Mainstream statistics courses often include statistical graphics: scatterplots, box-and-whisker plots, histograms, etc.  In R, these basic graphics can be made with \function{xyplot} and \function{bwplot}, for instance:
<<ex1,fig.width=2,fig.keep='all'>>=
require(mosaic)
xyplot(width ~ length,data=KidsFeet)
bwplot(width ~ sex,data=KidsFeet)
@

These things involving the \verb+~+ squiggle character --- ``tilde" --- are called \newword{formulas} in R. The word "formula" is used a somewhat different way than in algebra: the formula lays out how you want to relate variables: y versus x, what to break down by what.  

It may seem that this is just another way, among many, to invoke plotting software.  But  there's more going on here: R provides a {\em modeling language} an an easy path from basic graphics and basic statistics to multivariable modeling.  

The path is easy enough that it makes sense to start in that direction early: using the concepts, terminology, and techniques of modeling as a way of introducing statistics.  Here is what such a journey might look like.

\section{Formulas and Basic Statistics}

It's worthwhile to start a statistics course with graphics.  In addition to conveying well ideas of variation, graphics are close to the data themselves, compelling, and motivating.  Students see the advantage of using software; they couldn't make such graphics by hand.

\marginnote{Explanation of the \texttt{KidsFeet} dataset.  Why, what, where it is found.}
...
\chapter{From Formulas to Models}

You've already seen how the formula notation can be used in the calculation of statistics, for example, this calculation of grand means and groupwise means:
<<>>=
mean(width~1,data=KidsFeet) # Grand mean
mean(width~sex,data=KidsFeet) # Groupwise by sex
@

Means are considered basic.  But contrast them with a more advanced methodology: regression.  Here's a model of foot width against sex:
<<>>=
mod1 = lm(width~sex,data=KidsFeet)
@
...
\begin{itemize}
\item A model of this sort links inputs, called ``explanatory variables," to an output, or ``response variable". \sidenote{Depending on the field, these may be called dependent and independent variables respectively.} In the example above, \VN{width} is the response variable while \VN{sex} an the explanatory variable.
...
To make sense of this, it can be useful to construct the function explicitly and apply the inputs to it.
\marginnote{In this example, the input to the function is a category.  The possible values of a categorical variable --- the levels --- are often denoted in R as quoted character strings, \texttt{"G"} and \texttt{"B"} in the \texttt{KidsFeet} data, standing for girls and boys.}
<<>>=
f1 = makeFun(mod1)
@
This use of \function{makeFun} reformats the information that's already in \texttt{mod1}  as a mathematical function in R. Once that's done, the function can be evaluated in the ordinary way, by specifying the inputs.  For instance, you can ask what is the model function's value for each level of the explanatory variable \VN{sex}.
<<>>=
f1(sex="G")
f1(sex="B")
@

The function \texttt{f1} attempts to describe the relationship between the explanatory and response variables: between \VN{sex} and \VN{width}.
...
There are, \marginnote{There's nothing fundamental about \function{listFun}.  It's just a program that makes it easy to construct a mathematical function with categorical inputs.}
of course, other possible functions that could be used to describe the relationship between \VN{sex} and \VN{width}.  For example, here's a function that says the model value for boys is 10 cm, while that for girls is 9 cm:\authNote{Add listFun to mosaic package?}
<<>>=
fetchData("m155development.R")
f2 = listFun("B"=10,"G"=9)
@

Two model functions, \texttt{f1} and \texttt{f2}.  Which is better?
...
<<>>=
KidsFeet = transform(KidsFeet, f1val=f1(sex), f2val=f2(sex))
@
