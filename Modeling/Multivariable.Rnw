...
To load the \pkg{mosaic} package,
```{r}
require(mosaic)
```
...
\marginnote{We're starting with a data set, \texttt{trebuchet} collected by a student, Andrew Pruim, as part of a Science Olympiad competition.  These data are included in the \pkg{fastR} package.  

A trebuchet is a device used for throwing projectiles.  Introduced in medieval times, it was used as a siege weapon, throwing projectiles of several hundred pounds to destroy fortifications. 

\includegraphics[width=1.5in]{images/Trebuchet2.png}}
...
<<>>=
mean(distance~1,data=trebuchet) # Grand mean
mean(distance~object,data=trebuchet) # Groupwise by sex
@
...
\marginnote{In this example, the input to the function is a category.  The possible values of a categorical variable --- the levels --- are often denoted in R as quoted character strings, \texttt{"foose"} and \texttt{"golf"} in the \texttt{trebuchet} data, standing for a fooseball and a golf ball.}
<<>>=
f1 = makeFun(mod1)
@
