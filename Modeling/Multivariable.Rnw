...
HERE, I'm going to go back to the groupwise means and show that you can construct a function that works for a categorical input.

\subsection{Combinations of categorical and quantitative variables}

BASICALLY, the swimming data example.  I'll use the Galton height data.

\section{NOTES for the REST}
