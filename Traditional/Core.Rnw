...
Care) study: a randomized trial of a novel 
...
from the 
sixth edition of
Moore, McCabe and Craig's \emph{Introduction to the Practice of Statistics}\cite{moor:mcca:2007} (see \url{http://www.math.smith.edu/~nhorton/ips6e}) as well as
the third edition of the \emph{Statistical Sleuth}\cite{Sleuth3} (see \url{http://www.math.smith.edu/~nhorton/sleuth3}).
...
this modeling language commands throughout this document.
...
mean(HELPrct$cesd)
@
Similar functions exist for other summary statistics.
<<cesd2,cache=FALSE>>=
...
\Caution{Not all commands (including \function{quantile()}) have been upgraded to
support the formula interface. These must be accessed using \function{with()} or the \$ operator.}
<<cesd4>>=
...
The \function{xhistogram()} function is used to create a histogram.
...
<<women-cesd-hist,tidy=FALSE>>=
xhistogram(~ cesd, fit="normal",
  data=subset(HELPrct, sex=='female'))  
@

Alternatively, we can make side-by-side plots to compare multiple subsets.
<<cesd-male-female>>=
xhistogram(~ cesd | sex, data=HELPrct)
@

The layout can be rearranged.
\begin{center}
<<cesd-dotlayout>>=
xhistogram(~ cesd | sex, layout=c(1, 2), data=HELPrct)   
@
\end{center}
\begin{problem}
Using the \variable{HELPrct} dataset, 
...
\begin{solution}
<<subsmale, tidy=FALSE>>=
bwplot(cesd ~ substance, fit="normal", 
...
xhistogram(~ cesd, nint=20, data=female)   
@
\end{center}

Or the width can be specified.
\begin{center}
<<cesd-dotwidth,fig.height=1.8>>=
xhistogram(~ cesd, width=1, data=female)   
@
\end{center}
...
xhistogram(~ cesd, data=HELPrct, subset=(sex=='female'))   
@
\end{center}
...
a vertical line. A variety of line types can be specified, 
...
with(female, ladd(panel.mathdensity(args=
  list(mean=mean(cesd), sd=sd(cesd)), col="red")))
ladd(panel.abline(v=60, lty=2, lwd=2, col="grey"))
@
\end{center}
\DiggingDeeper{The \function{plotFun()} function can also be used to annotate plots.}
...
<<tinterval>>=
with(female, t.test(cesd))
confint(with(female, t.test(cesd)))
@

But it's also straightforward to calculate this using a bootstrap 
(using the approach described in the \pkg{mosaic} package Resampling Vignette).
...
mean(~ cesd, data=resample(female))
@
\DiggingDeeper{Even though a single trial is of little use, it's smart have
...
with(trials, quantile(result, c(.025, .975)))
@
...
tally(~ homeless, margins=FALSE, data=HELPrct)      
...
This takes a table (as generated by \function{tally()} with the {\tt margins=FALSE} option
as an argument).  
<<tidy=FALSE>>=
prop.test(tally(~ homeless=="housed", margins=FALSE, 
...
<<>>=
prop.test(~ homeless, data=HELPrct)
@
...
substab <- tally(~ substance, margins=FALSE, data=HELPrct)
substab
@
<<>>=
...
These use the same formula interface introduced previously for statistics and graphics 
to specify the outcome 
and predictors.  Here we consider fitting the model \model{\variable{cesd}}{\variable{mcs}}.


<<>>=
model <- lm(cesd ~ mcs, data=HELPrct)
coef(model)
@
...
\begin{center}
<<HELPrct-pred-plot,tidy=FALSE>>=
xyplot(cesd ~ mcs, panel=panel.lmbands, cex=0.2, 
  data=HELPrct)
@
\end{center}
...
\begin{problem}
Using the HELPrct dataset, fit a simple linear regression model 
...
<<tidy=FALSE>>=
oddsRatio(tally(~ homeless + sex, margins=FALSE, 
  data=HELPrct)) 
@
Note that the reference group is flipped (and that $1/1.599 = \Sexpr{1/1.599}$).
...
an association this strong if homeless status and sex were independent back in the 
population.
...
%with(HELPrct, fisher.test(homeless, sex))
<<help-fisher>>=
...
\section{A Bivariate predictor: numerical and graphical summaries}
...
<<>>=
compareMean(age ~ shuffle(sex), data=HELPrct)
compareMean(age ~ shuffle(sex), data=HELPrct)
@

<<permute-HELPrct,tidy=FALSE>>=
rtest.stats = do(500) * compareMean(age ~ shuffle(sex), 
  data=HELPrct)
xhistogram(~ result, n=40, xlim=c(-6, 6),
  groups=result >= test.stat, pch=16, cex=.8, 
  data=rtest.stats)
ladd(panel.abline(v=test.stat))
@

Here we don't see much evidence to contradict the null hypothesis that the populations
have the same mean age back in the population.
...
compare
@
<<help-hsd3,fig.height=4>>=
plot(compare,cex.lab=0.7)
@
...
\FoodForThought{The \function{glm()} function has arguments \option{family}  and
...
\begin{center}
<<help-km,fig.width=6,fig.height=3.9,tidy=FALSE>>=
require(survival)
...
bwplot(cesd ~ substance | sex, data=HELPrct)
@
<<help-aov2>>=
...
Multiple regression is a logical extension of the prior commands, adding 
additional predictors (and allowing students to start to try to disentangle 
multivariate relationships).  

\InstructorNote{We also tend to introduce multiple linear regression
...
<<help-multreg>>=
lm1 <- lm(cesd ~ mcs + age + sex, data=HELPrct)
summary(lm1)
@
We can also fit a model that includes an interaction between MCS and sex.
<<help-multreg2>>=
lm2 <- lm(cesd ~ mcs + age + sex + mcs*sex, data=HELPrct)
summary(lm2)
@
<<help-multreg3>>=
anova(lm1, lm2)
...
display the predicted CESD values for a 36 year old male and female subject from the parallel
slopes model.
<<makeFUN>>=
lm1fun = makeFun(lm1)
@
We can now plot this function for a range of values for MCS (mental component score).  
<<plotFUN,tidy=FALSE>>=
plotFun(lm1fun(mcs, age=36, sex="male") ~ mcs, 
  xlim=c(0,60), lwd=2, ylab="predicted CESD")
plotFun(lm1fun(mcs, age=36, sex="female") ~ mcs, 
...
<<>>=
HELPrct = transform(HELPrct, resids = resid(lm1))
HELPrct = transform(HELPrct, pred = fitted(lm1))
@
<<tidy=FALSE>>=
xhistogram(~ resids, xlab="residuals", fit="normal", 
  data=HELPrct)
@
We can display the observations with the extremely large negative residual.
<<>>=
subset(HELPrct, resids < -25)
@

<<tidy=FALSE>>=
xyplot(resids ~ pred, ylab="residuals", cex=0.3,
  xlab="predicted values", main="predicted vs. residuals",
  type=c("p", "r", "smooth"), data=HELPrct)
@
<<tidy=FALSE>>=
xyplot(resids ~ mcs, xlab="mental component score", 
  ylab="residuals", cex=0.3,
  type=c("p", "r", "smooth"), data=HELPrct)
@
The assumptions of normality, linearity and homoscedasticity seem reasonable here.
\begin{problem}
The {\tt RailTrail} dataset within the \pkg{mosaic} package includes the counts 
...
Describe the distribution of the variable {\tt avgtemp} in terms of its
...
\begin{problem}
The {\tt RailTrail} dataset also includes a variable called {\tt cloudcover}.  
...
\begin{problem}
The variable in the {\tt RailTrail} dataset that provides the daily count
of crossings is called {\tt volume}. 
Describe the distribution of this variable in terms of its
center, spread and shape.
\end{problem}
...
\begin{problem}
The {\tt RailTrail} dataset also contains an indicator of whether the day was 
a weekday ({\tt weekday==1}) or a weekend/holiday ({\tt weekday==0}).
...
\begin{problem}
Use side-by-side boxplots to compare the distribution of {\tt volume} by day type in the {\tt RailTrail} dataset.
Hint: you'll need to turn the numeric {\tt weekday} variable into a factor variable using \function{as.factor()}.
...
\begin{problem}
Use overlapping densityplots to compare the distribution of {\tt volume} by day type in the {\tt RailTrail} dataset.
What do you conclude?
\end{problem}
...
\begin{problem}
Create a scatterplot of {\tt volume} as a function of {\tt avgtemp} using the {\tt RailTrail} dataset, along with a regression line and scatterplot
...
\begin{problem}
Using the {\tt RailTrail} dataset,
fit a multiple regression model for {\tt volume} as a function of {\tt cloudcover}, {\tt avgtemp}, {\tt weekday} and the interaction
between these day type and average temperature.
...
on weekdays and weekends/holidays for average temperatures between 30 and 80 degrees and a cloudy day ({\tt cloudcover=10}).  
\end{problem}
\begin{solution}
<<>>=
myfun = makeFun(fm)
...
To provide a general check (or for future calculations of this sort) we can use the 
...
data are truly normal.  \DiggingDeeper{It's useful to have students calculate power empirically, 
...
Data management is a key capacity to allow students (and instructors) to ``compute with data''.
...
\DiggingDeeper{It's a good idea to start teach good practices for choice of variable names from day one.}
<<>>=
names(faithful)
@
...
\DiggingDeeper{Unfortunately, not all functions in R support the \function{subset=} or
...
We begin by detaching the \pkg{fastR} package.  Then we can create a categorical variable
at cuts at 20 and 40.
<<tidy=FALSE>>=
detach("package:fastR")
favstats(~ cesd, data=HELPrct)
...
\variable{anysub} & use of any substance post-detox & see also {\tt daysanysub}
\\ \hline
...
\variable{daysanysub} & time (in days) to first use of any substance post-detox (range 0--268)  & see also {\tt anysubstatus} \\ \hline
\variable{dayslink} & time (in days) to linkage to primary care (range 0--456)  & see also {\tt linkstatus}
\\ \hline
\variable{drugrisk} & Risk-Assessment Battery (RAB) drug risk score  (range 0--21)  & see also {\tt sexrisk}
\\ \hline
...
\\ \hline
\variable{i1} & average number of drinks (standard units) consumed per day (in the past 30 days, range 0--142) & see also {\tt i2}
\\ \hline
\variable{i2} & maximum number of drinks (standard units) consumed per day (in the past 30 days range 0--184) & see also {\tt i1}
\\ \hline
...
\\ \hline
\variable{linkstatus} & post-detox linkage to primary care (0=no, 1=yes)  & see also {\tt dayslink}
\\ \hline
\variable{mcs} & SF-36 Mental Component Score  (range 7-62, higher scores are better)  & see also {\tt pcs}
\\ \hline
\variable{pcs} & SF-36 Physical Component Score  (range 14-75, higher scores are better)  & see also {\tt mcs}
\\ \hline
...
\variable{sexrisk} & Risk-Assessment Battery (RAB) sex risk score  (range 0--21)  & see also {\tt drugrisk}
\\ \hline
