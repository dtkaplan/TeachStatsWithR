...
<<>>=
prop.test(xtabs(~ homeless=="housed", HELPrct), correct=FALSE)
@
...
distribution.  For the HELPrct data, we could test the null hypothesis that the 
proportion of subjects back in the original population in each of the 
substance abuse groups were equal.

<<gof>>=
substab <- tally(~ substance, data=HELPrct)
substab
@
<<gofmore>>=
...
\begin{center}
<<HELPrct-xyplot,fig.height=3>>=
xyplot(cesd ~ mcs, data=HELPrct, type=c('p','smooth'), pch=1)
@
\end{center}

For multivariate data sets, \function{splom()} will generate pairwise
scatterplots for selected variables.
<<HELPrct-splom,fig.height=4>>=
splom(subset(HELPrct, keep=c("age", "cesd", "d1")))
@
...
\begin{multicols}{2}
\begin{center}
<<HELPrct-lmplot1,fig.width=3,fig.height=4>>=
plot(model, which=1)
@
<<HELPrct-lmplot2,fig.width=3,fig.height=4>>=
plot(model, which=2)
@
\end{center}
\end{multicols}
...
<<homeless-sex>>=
xtabs(~ homeless + sex, data=HELPrct)     
@



\authNote{Need to decide how we want to create datas from summaries}

XX Epidemiologic statistics
...
with(HELPrct, odds.ratio(homeless, sex))
@

The \pkg{mosaic} package has a function which will calculate odds ratios:
<<>>=
oddsRatio(tally(~ homeless + sex, data=HELPrct)) 
@

XX Graphical summaries
...
\begin{center}
<<mosaicplot,fig.height=3.5>>=
mytab <- tally(~ homeless + sex, margins=FALSE, data=HELPrct)
mosaicplot(mytab)
@
\end{center}
...
%with(HELPrct, chisq.test(homeless, sex, correct=FALSE))
<<chisq1>>=
chisq.test(xtabs(~ homeless + sex, data=HELPrct), correct=FALSE)
@
...
<<chisq2>>=
xchisq.test(xtabs(~homeless + sex, data=HELPrct), correct=FALSE)
@
...
<<aggregate>>=
aggregate(cesd ~ sex, data=HELPrct, FUN=mean)
@

The \function{summary()} function can provide more statistics by group.
<<summary>>=
summary(cesd ~ sex, data=HELPrct, fun=favstats)
@
...
\authNote{Need to update using new functions}
%rtest.stats[simulations+1,] <- test.stat  # add in the observed value
<<permute-HELPrct>>=
simulations <- 500
test.stat <-  abs(diff(aggregate(cesd ~ sex, data=HELPrct, mean)$cesd))
test.stat
rtest.stats <- do(simulations) * c(diff= diff(aggregate(cesd ~ shuffle(sex), HELPrct, mean)$cesd))
...
<<aggregate2>>=
aggregate(cesd ~ substance, data=HELPrct, FUN=mean)
@
<<help-aov>>=
...
\begin{center}
<<help-km,fig.width=6,fig.height=3.9>>=
library(survival)
fit <- survfit(Surv(dayslink, linkstatus) ~ treat, data=HELPrct)
...
<<help-surv>>=
library(survival)
summary(coxph(Surv(dayslink, linkstatus) ~ age + substance, data=HELPrct))
@
...
<<help-interaction>>=
xyplot(cesd ~ substance, data=HELPrct, groups=sex, type='a')
@
...
<<probdist>>=
xpnorm(1.96, 0, 1)    # P(Z < 1.96)
qnorm(.975, 0, 1)
integrate(dnorm, -Inf, 0)
@

The following displays the basenames for probability distributions 
...
<<mr-adding-variable>>=
iris <- transform(iris, Length = cut(Sepal.Length, 4:8))    # cut places data into bins
@
...
We will focus on \verb!HELPrct!, which contains
