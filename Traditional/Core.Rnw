...
We will use data from the HELP (Health Evaluation and Linkage to Primary 
...
Other related resources may be helpful, including an annotated set of examples
from the 
sixth edition of
Moore, McCabe and Craig's \emph{Introduction to the Practice of Statitics}\cite{moor:mcca:2007} (see \url{http://www.math.smith.edu/~nhorton/ips6e}) as well as
...
options(digits=3)
@
Note that the \function{mean()} in the \pkg{mosaic} package supports a modeling language
common to \pkg{lattice} graphics and linear models (e.g. \function{lm()}).  We will use
these commands throughout this document.
...
In the \VN{HELPrct} dataset, approximately one quarter of the subjects are female.  
<<>>=
...
Or the width can be constrained.
\begin{center}
...
dotPlot(~ cesd, xlab="CESD score", data=subset(HELPrct, 
...
ladd(panel.abline(v=60, lty=2, lwd=3, col="green"))
@
\end{center}
...
Missing values arise in almost all real world investigations.  R uses the {\tt NA} character as an 
indicator for missing data.  The {\tt HELPmiss} dataframe within the \pkg{mosaic} package includes all 
$n=470$ subjects enrolled at baseline (including the $n=17$ subjects with some missing data who 
were not included in {\tt HELPrct}).  
<<tidy=FALSE>>=
...
\caption{Annotated description of variables in the \VN{HELPrct} dataset}
\label{tab:helpvars} \\
\hline
...
\\ \hline
{\tt cesd} & Center for Epidemiologic Studies Depression scale (range 0--60, higher scores indicate more depressive symptoms)  & \\ \hline
...
\\ \hline
{\tt mcs} & SF-36 Mental Component Score  (range 7-62, higher scores are better)  & see also {\tt pcs}
\\ \hline
{\tt pcs} & SF-36 Physical Component Score  (range 14-75, higher scores are better)  & see also {\tt mcs}
\\ \hline
{\tt pss\_fr} & perceived social supports (friends, range 0--14) & 
\\ \hline
{\tt racegrp} & race/ethnicity (black, white, hispanic or other)  &  \\ \hline
