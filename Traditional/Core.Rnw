...
mean(~ cesd, data=resample(female))
@
\TeachingTip{Even though a single trial is of little use, it's smart have
...
\Caution{The \option{margins=FALSE} option is needed here to include only the counts.}
<<gofmore,tidy=FALSE>>=
tally(~ substance, format="percent", data=HELPrct)
observed <- tally(~ substance, margins=FALSE, data=HELPrct)
observed
@
<<tidy=FALSE>>=
...
\TeachingTip{We don't encourage much manual calculations in our courses.}
<<gof2,tidy=FALSE>>=
...
of the CESD (a measure of depressive symptoms, higher scores indicate more symptoms) and the MCS (mental component score from the SF-36, where higher scores indicate better functioning)
...
regarding their utility, relative to the low data to ink ratio\cite{Tufte:2001:Visual}).  
...
The groups can also be compared using the \function{lm()} function (with an equal variance assumption).
<<>>=
...
anova(mod2)
@
The \function{anova()} command can also be used to formally 
...
Significant Difference (HSD).  Other options are available within the 
...
@
<<help-hsd2>>=
mod = lm(cesd ~ substance, data=HELPrct)
...
\FoodForThought{The \function{glm()} function has arguments \option{family}, which can take an option 
...
\authNote{Add a plot with logistic fit overlaid?}
...
lm1fun = makeFun(lm1)
@
We can now plot this function for a range of values for MCS (mental component score), along
...
<<>>=
HELPrct = transform(HELPrct, residuals = resid(lm1))
HELPrct = transform(HELPrct, pred = fitted(lm1))
@
<<tidy=FALSE>>=
xhistogram(~ residuals, xlab="residuals", fit="normal", 
  data=HELPrct)
@
We can display observations with extremely large residuals.
<<>>=
...
@
<<tidy=FALSE>>=
plotDist('binom', params=list(size=25, prob=0.25), 
  xlim=c(-1,26))
@
\begin{problem}
...
Let $X_1, ..., X_{25}$ be i.i.d. $N(0.3, 1)$ (this is the alternate that we wish to calculate power for).  Consider testing the null hypothesis $H_0: \mu=0$ versus $H_A: \mu>0$ at significance level $\alpha=.05$.  We will compare the power of the sign test and the power of the test based on normal theory (one sample one sided t-test) assuming that $\sigma$ 
is known.
...
\Caution{Unfortunately, not all functions in R support the \function{subset=} or
\option{data=} options.}
...
We begin by detaching the \pkg{fastR} package (which redefined the \function{favstats()} function.  
