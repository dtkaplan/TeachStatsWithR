...
the \emph{Start Teaching with R} and \emph{Start Modeling with R} books that are freely available as part of the 
\pkg{mosaic} package.
...
\pkg{mosaic} package, 
which was written to simplify the use of \R for introductory statistics courses, and the 
\pkg{mosaicData} package which includes a number of data sets. 
A short summary of the \R\ commands needed to teach introductory statistics can be found in 
the mosaic package vignette 
(\url{http://cran.r-project.org/web/packages/mosaic/vignettes/mosaic-resources.pdf}).
...
\pkg{mosaic} and \pkg{mosaicData} packages can be installed using the following commands:
...
commands using variants of this modeling language throughout this document.  Those already familiar with \R\ may be surprised by the form of this command.
...
<<echo=FALSE>>=
#detach(package:MASS)   # where did MASS come from?  (histogram in mosaic XX)
@
...
<<probdist,fig.height=4,fig.width=6, tidy=FALSE>>=
...
\TeachingTip{The \function{cut()} function has an option called \option{labels} which can be used to specify more descriptive names for the groups.}
...
\emph{TCF7L2}.  
The \dataframe{pheno} dataframe contains phenotypes
(including type 2 diabetes case/control status) for an intersecting set of individuals.
We can join (or merge) these together to explore the association between
...
single state in a particular year.
...
Variables included in the HELP dataset are described in Table \ref{tab:helpvars}.  More information can be found here\cite{horton:kleinman:2015}.
