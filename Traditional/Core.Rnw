...
the materials related to XX STARTING R, MODELING and RESAMPLING XX.
...
to rumor, some of us do on occasion.  Nick has used the 6th edition of
Moore and McCabe \cite{moor:mcca:2007}, where he introduces least squares 
...
with(HELPrct, quantile(cesd, c(.25, .50)))
@
...
<<women-cesd-hist>>=
xhistogram(~ cesd, data=subset(HELPrct, sex=='female'))   # notice the quotes
@
...
\begin{solution}
<<subsmale>>=
bwplot(cesd ~ substance, data=subset(HELPrct, sex=='male'))
@
\end{solution}
...
xhistogram(~ cesd, nint=20, data=female)   # was dotPlot
@
\end{center}
...
xhistogram(~ cesd, data=HELPrct, subset=(sex=='female'))   # was dotPlot
@
\end{center}
...
\begin{center}
<<cesd-dot3>>=
xhistogram(~ cesd | sex, data=HELPrct)   # was dotPlot
@
\end{center}
...
ladd(panel.mathdensity(args=list(mean=mean(female$cesd), sd=sd(female$cesd)),
  col="red"))
ladd(panel.abline(v=60, col="green"))
@
\end{center}
...
(using the approach described in chapter \ref{chap:simulation}).
...
trials = do(1000) *
  with(resample(female), mean(cesd))
quantile(trials$result, c(.025, .975))
@
...
<<homeless-table>>=
tally(~ homeless, data=HELPrct)      # display table using counts
tally(~ homeless, format="percent", data=HELPrct)      # display table using percentage
tally(~ homeless, format="proportion", data=HELPrct)      # display table using proportion
@
...
<<help-hsd>>=
with(HELPrct, tapply(cesd, substance, mean))
@
<<help-hsd2>>=
...
<<help-logit>>=
logitmod <- glm(homeless ~ age + female, family=binomial, data=HELPrct)
summary(logitmod)
exp(coef(logitmod))
@
