...
\pkg{mosaic} package, which was written to simplify the use of R for introductory statistics courses. A short summary of the R needed to teach introductory statistics can be found in the mosaic package vignette (\url{http://cran.r-project.org/web/packages/mosaic/vignettes/mosaic-resources.pdf}).
...
A pairs plot can be calculated for each pair of a set of variables.
...
The \function{mplot()} can facilitate creating a variety of useful plots, including residuals vs. fitted:
<<>>=
...
@
\end{center}
% XX Randy, any idea what's going on here with the panel.lmbands example?
...
A comparison of two models can be undertaken (for multiple degree of freedom tests).  For example, we
...
mymod1 = glm(homeless=="homeless" ~ age + substance, family=binomial, 
  data=HELPrct)
mymod2 = glm(homeless=="homeless" ~ age, family=binomial, data=HELPrct)
summary(mymod1)
exp(coef(mymod1))
anova(mymod2, mymod1, test="Chisq")
@
We observe that the cocaine and heroin groups are significantly less likely to be homeless than alcohol involved subjects, after controlling for age.  (A similar result is seen when considering just homelessless and substance.)

<<>>=
tally(~ homeless | substance, format="percent", data=HELPrct)
@
