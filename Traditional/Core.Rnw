...
The \function{filter()} function in the \pkg{dplyr} package can be used to generate a new data frame containing
...
The row names of a data frames can be changed by
...
\label{sec:subsets}
We can also use \function{filter()} to reduce the size of a data frame by selecting 
only certain rows.
\begin{center}
<<"mr-faithful-long-xy">>=
data(faithful)
