...
smallHELP = filter(females, select=c(cesd, mcs, pcs))
cor(smallHELP)
@
...
HELPrct = mutate(HELPrct, pred = fitted(lmnointeract))
@
<<tidy=FALSE>>=
...
iris <- mutate(iris, Length = cut(Sepal.Length, 4:8))    
@
...
names(CPS85)
find("select")
CPS1 <- dplyr::select(CPS85, select = -matches("workforce.years"))
names(CPS1)
@
...
of variable names.
<<>>=
CPS1 <- dplyr::select(CPS85, select = -matches("workforce.years|exper"))
@
...
simple assignment using \function{names()} or \function{row.names()}, respectively.
<<>>=
names(CPS85)
names(CPS85)[names(CPS85)=="workforce.years"] = "workforce"
names(CPS85)
@
...
<<tidy=FALSE>>=
smaller = dplyr::select(HELPmiss, cesd, drugrisk, indtot, mcs, pcs, substance)
dim(smaller)
summary(smaller)
@
