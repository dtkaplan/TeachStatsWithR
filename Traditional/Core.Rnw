...
these commands throughout this file.
\DiggingDeeper{The \emph{Start Modeling with R} will be helpful if you are unfamiliar with the 
modeling language}

The same output could be 
created using the following commands (though we will prefer the MOSAIC version):
<<>>=
...
The \function{tally()} can be used to calculate
...
<<tidy=FALSE>>=
prop.test(xtabs(~ homeless=="housed", HELPrct), 
  correct=FALSE)
@
...
\begin{center}
<<HELPrct-pred-plot>>=
xyplot(cesd ~ mcs, panel=panel.lmbands, cex=0.2, data=HELPrct)
@
\end{center}
...
<<chisq1,tidy=FALSE>>=
chisq.test(xtabs(~ homeless + sex, data=HELPrct), 
  correct=FALSE)
@
...
<<chisq2,tidy=FALSE>>=
xchisq.test(xtabs(~homeless + sex, data=HELPrct), 
  correct=FALSE)
@
...
<<help-fisher>>=
fisher.test(xtabs(~homeless + sex, data=HELPrct))
@
...
We see that the subjects in the treatment (HELPrct clinic) were significantly more likely to 
...
xyplot(cesd ~ substance, data=HELPrct, groups=sex, 
  type='a')
@
...
Here we consider a model for depressive symptoms as a function of Mental Component Score,
...
xhistogram(~ resids, xlab="residuals", fit="normal", 
  data=HELPrct)
@
We can display the observations with extremely large negative residuals.
<<>>=
subset(HELPrct, resids < -25)
@

<<tidy=FALSE>>=
xyplot(resids ~ pred, xlab="residuals", 
  ylab="predicted values", cex=0.3,
...
@
<<>>=
qnorm(.975, mean=0, sd=1)   #  value which satisfies P(Z < z) = 0.975
