...
opts_chunk$set( fig.path="figure/SimulationBased-fig-" ) 
standalone=TRUE
...
\authNote{Add sidebars and definitions of permutation and bootstrap}
...
\authNote{RJP:
No one will get the same state twice with \texttt{sample()} or 
...
set.seed(123)
@
\authNote{Line width request not being respected}
<<>>=
...
<<>>=
do(1) * lm(age ~ sex, HELPrct)            # using the data as is
@
But it really shines when we start looking at sampling distributions.
<<>>=
do(3) * lm(age ~ shuffle(sex), HELPrct)   # simulation under a null hypothesis
@

\subsection{Generating Fake Data For Sampling Activities}
\label{sec:fake-families}%
While we (in line with the GAISE guidelines) always encourage use of actual
data, there may be times when being able to 
\authNote{sidebar for GAISE plus preface?}
simulate data
can help illustrate a point.  This is also an important capacity for modeling.
...
From this we see that it is not that unlikely to see a count of 138 or more.
(See samples \Sexpr{ paste(which(apply(rgolfballs, 1, max) >= 138), collapse=', ') }.)
Students are often surprised just how often this occurs.
\authNote{review definitions and flow for test statistics}
...
\begin{center}
<<golfballs-max,fig.height=1.7>>=
statTally(golfballs, rgolfballs, max)
ladd(panel.abline(v=138))                # add a vertical line
@
\end{center}
...
<<eval=FALSE>>=
statTally(golfballs, rgolfballs, min)   # output surpressed.
@
...
\begin{center}
<<golfballs-range,fig.height=1.7>>=
mystat1 <- function(x) { diff(range(x)) }
statTally(golfballs, rgolfballs, mystat1, v=mystat1(golfballs)) # add a vertical line
@
\end{center}

\subsubsection{The World's Worst Test Statistic}

Sometimes a student will suggest what turns out to be the world's worst test statistic:
the sum of the differences between the counts and $486/4 = 121.5$.
\begin{center}
<<golfballs-worst,eval=FALSE,fig.height=1.7>>=
mystat2 <- function(x) { sum(x - 121.5) }
statTally(golfballs, rgolfballs, mystat2)
@
\end{center}
...
\begin{center}
<<golfballs-abs,fig.height=1.7>>=
sad <- function(x) { sum(abs(x-121.5)) }
statTally(golfballs, rgolfballs, sad, v=sad(golfballs))
@
\end{center}
...
\begin{center}
<<golfballs-var,fig.height=1.7>>=
statTally(golfballs, rgolfballs, var, v=var(golfballs))
@
\end{center}
...
\begin{center}
<<resample-mean-run>>=
xhistogram(~ result, resample.means)
@
\end{center}
...
proportions, regression coefficients, etc.  These are also more
interesting.
...
\begin{center}
<<Whickham-smokers>>=
xhistogram(~ Yes, null.dist, main = 'Sampling distribution of smoking proportion')
@
\end{center}
\end{example}

\begin{example}
\authNote{compareMean()?}
...
\authNote{compare proportions without tricking lm()}
...
@ 
\authNote{add t.test here}
\authNote{sidebar on confounding?}
...
failure to take into account other factors that might be confounding this relationship.  What's shocking is that we
teach students about p-values without teaching them about covariates
and how to adjust for them using the language of models described in XX.
...
effect of \VN{smoker} status.  We consider this more in Chapter
...
You might have noticed that the model built here was a logistic regression
...
The US Space Shuttles carried out their last mission in 2011
before being retired.  For a few years, at
...
In this section we show several ways to see if the mean ages of men and women in the HELPrct study 
...
lm(age ~ sex, data = HELPrct)                        # actual data
@
...
<<>>=
do(1) * lm(age ~ sex, data = HELPrct)                # actual data
do(2) * lm(age ~ shuffle(sex), data = HELPrct)       # shuffled data
@
...
null.dist <- do(1000) * lm(age ~ shuffle(sex), HELPrct) 
test.stat <- (do(1) * lm(age ~ sex, HELPrct) )$sexmale 
...
\authNote{update with new material}
\begin{center}
<<two-sample-hist,fig.width=5>>=
xhistogram(~ rtest.stats, v=test.stat, groups=rtest.stats >= test.stat)      
@
\end{center}

\begin{center}
<<two-sample-dot,fig.width=5,fig.height=2.2>>=
test.stat <-  diff(aggregate(age ~ sex, HELPrct, mean)$age)
rtest.stats <- do(200) * c(diff= diff(aggregate(age ~ shuffle(sex), HELPrct, mean)$age))
xhistogram(~ diff, data = rtest.stats, 
...
<<help101>>=
summary(age ~ sex, data = HELPrct, fun = mean)
@
...
<<help102>>=
aggregate(age ~ sex, data = HELPrct, FUN = mean)
@
...
\authNote{What if more than 2 groups?}
<<>>=
do(1) * mean(age ~ sex, data = HELPrct)
@
...
<<help103>>=
diff(aggregate(age ~ sex, HELPrct, mean )$age)                         # actual
with(aggregate(age ~ sex, HELPrct, mean ), diff(age))                  # actual
with(do(1) * mean(age ~ sex, data = HELPrct), mean.female - mean.male)   # actual
@
\authNote{Need to fix this...}
...
\Sexpr{round(diff(aggregate(age ~ sex, HELPrct, mean )$age),2)} with
the values we obtain if we randomly shuffle the \variable{sex} variable.

\authNote{Need to check this is working}
<<help104>>=
with(do(1) * mean(age ~ sex, data = HELPrct), mean.female - mean.male)    # actual
null.dist <- do(3) * mean(age ~ shuffle(sex), data = HELPrct); null.dist   # shuffled
...
test.stat <- with(do(1) * mean(age ~ sex, data = HELPrct), mean.female - mean.male)  # actual
null.dist <- do(1000) * mean(age ~ shuffle(sex), data = HELPrct); head(null.dist)     # shuffled
...
data.t <- stat(t.test(age ~ sex, HELPrct)); data.t
null.dist <- do(1000) * stat(t.test(age ~ shuffle(sex), HELPrct))
...
\begin{center}
<<t-test-simulation-histogram,fig.width=5>>=
