...
But there are some additional things that instructors should know about
\R.  We outline some of these things in this chapter.
...
\begin{itemize}
  ...
  scripts, RMardown files, and Rnw files.

You can execute all the code in an \R\ script file using 
\marginnote{\R\ can be used to create executable scripts.  Option parsing and 
...
See the buttons in the panel for any \R\ script, RMardown or Rnw file.  
...
(or the console) using the History panel.

  \item Use meaningful names.

	  Rarely should objects be named with a single letter.  
	  
	  Adopt a personal convention regarding case of letters to you have one
	  less thing to remember when trying to recall the name of an object.  For
...
If you are working in RMardown or Rnw files, you can also include nicely
...
data import.
In this section we primarily discuss ways to create and manipulate 
data within \R.   But first discuss a few more details regarding importing data.
...
build a data frame for our data as follows.
...
The example below queries a the UCSC genome browser to find the first
5000 genes on chromosome~1.

\begin{table*}
<<ucsc-sql,tidy=FALSE>>=
require(RMySQL)
 
# Set up a connection to the public MySQL server for the UCSC genome browser (no password)
...
# Get the UCSC gene name, start and end sites for the first 5000 genes on Chromosome 1
results <- 
  query("SELECT name, chrom, txStart, txEnd FROM mm9.knownGene WHERE chrom='chr1' LIMIT 5000;")
dim(results)
head(results,2)
@
\end{table*}
...
approximate shape of some distribution.  One way to do that was illustrated above.
<<>>=
x1 <- rnorm(500, mean=10, sd=2)
xhistogram(~x1, width=.5)
@
...
The \function{transform()} function can be used to add or modify variables in a data frame.
<<>>=
results <- transform(results, length=(txEnd - txStart)/1000)
head(results, 2)
xhistogram( ~length, data=results, xlab="gene length (kb)" )
@
...
\subsection{Object not found}

\subsection{Other error messages here}
