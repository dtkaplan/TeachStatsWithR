...
\marginnote{The \function{seq()} function
creates an arithmetic sequence of numbers from
a specified starting point, upper bound, and increment.}
...
The following functions produce vectors, but not by applying a 
function component by component.
<<vectors02b>>=
x
...
head(chrom1, 2)
xhistogram( ~log10(length), data=chrom1, 
            xlab="gene length (kb)" )
@
...
of years of education obtained.  
<<>>=
...
\marginnote{An alternative solution is to use the \dataframe{geyser} data set in
...
\label{sec:subsets}
We can use \function{subset()} to select 
...
\Rindex{mystats()}%
\Rindex{function()}%
\marginnote[-.5in]{There are ways to check the \rterm{class} of an argument
