...
\Pointer[-2cm]{\R\ can be used to create executable scripts.  Option parsing and 
...
\Pointer[-2cm]{Many objects also have \emph{attributes} which contain
additional information about the object, but unless you are 
doing programming with these objects, you don't need to
...
\Pointer[-2cm]{The \function{read.file()} function in the \pkg{mosaic} package
uses this as its default for \option{na.strings}.}
\Rindex{read.file()}
\Rindex{read.csv()}
\Rindex{read.table()}
...
file formats:
\Pointer[-2cm]{If you want to save an \R\ object but not its name, you can 
...
@
\Rindex{mutate()}
\Caution[1cm]{The arithmetic operations in this \function{mutate()} command are being executed in SQL, not in \R, and the palette of allowable functions is much smaller. It is not possible, for example, to compute the logarithm of the length here using \function{log()}.  For that we must first collect the data into a real data frame.}
<<>>=
Chrom1 %>% 
