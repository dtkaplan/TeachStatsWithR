...
In fact this is what was done for all but one of the cases to create the \variable{exper} variable that is already in the \dataframe{CPS85} data.
<<>>=
...
HELP3 <- mutate(HELPrct, 
  risklevel = derivedFactor(
    low = sexrisk < 5, medium = sexrisk < 10,
	  high = sexrisk >=10, .method = "first"      # use first rule that applies
	)
)
head(HELP3, 4)
@
...
\noindent The difference between an inner join and a left join is that the inner join 
...
\Caution{The \pkg{RMySQL} package is not as easy to install as other R packages.  \pkg{RMySQL} has to be installed from ``source," and it requires installation of other software such as \code{gcc}.  Search on the web for "install RMySQL" and follow the instructions you find there.  It will help to have someone proficient with {\sc unix} to give you pointers.} %
...
<<tidy=FALSE>>=
require(RMySQL) # load the package
...
# Get the gene name, chromosome, start and end sites 
#  for genes on Chromosome 1
Chrom1 <-
  KnownGene %>% 
...
{\it Getting the URL of a file in a Dropbox Public directory}
\end{center}
...
\Caution{GoogleDocs occasionally changes its user interface.  A recent change eliminated publishing spreadsheets in CSV format.  To make a new spreadsheet that can be published as CSV, use this link: \url{https://g.co/oldsheets}.  

Use the "Publish to the Web" menu item to make the spreadsheet readable by your students.  Then, under "Get a link to the published data," choose the CSV format.  You'll need to provide the link URL generated by Google to your students.  This URL is so long and awkward, that you will want to provide it in a form that students can cut and paste.

If you want your students to be able to edit the Google spreadsheet, use the standard "Share" interface.  We find it best to choose "anyone with the link" so that you don't need to list all of your students' accounts.  Then publish the link on your syllabus.

Another option is to get shorter URLs using 
...
elev <- fetchGoogle(
"https://spreadsheets.google.com/spreadsheet/pub?hl=en&hl=en&key=0Am13enSalO74dEV
zMGJSMU5TbTc2eWlWakppQlpjcGc&single=TRUE&gid=0&output=csv")
@
