...
\begin{itemize}
  ...
  scripts, RMarkdown files, and Rnw files.
...
See the buttons in the tab for any \R\ script, RMardown or Rnw file.  
...
One of the advantages to learning the \pkg{dplyr} system is that it scales well.  The 
same commands that can be applied to relatively small data frames can also be applied
to very large data sets and even to data stored in a data base.
...
<<tidy=FALSE>>=
head(chrom1, 2)
chrom1 <- mutate(chrom1, length=(txEnd - txStart)/1000)
head(chrom1, 2)
