...
\Pointer[-2cm]{Even when we only have a single integer, \R\ will treat it like
...
same type.  
\DiggingDeeper[-1cm]{In fact, they must all be of the same \emph{atomic} type.  
...
Some additional useful functions are included in Table~1.

\marginnote{Table~1. Some useful \R\ functions.}
...
\marginnote{Even if you primarily use the \RStudio\ interface to import data, it is good to know about the command line methods since these are required to import data into scripts, RMarkdown, and \pkg{knitr}/\LaTeX\ files.}
...
\Caution[-1cm]{
Be sure when using \function{scan()} that you remember to save your data somewhere. Otherwise you will have to type it again.}
...
For example, to simulate selecting a sample of size 12 from a normal population with mean 100 and standard deviation 10, use
\Rindex{rnorm()}
...
%in this chapter).
\Pointer{The pkg{lubridate} package provides a 
...
\marginnote[-1cm]{
If the variable containing a data frame is modified or used to store a different object, the original data from the package can be recovered using \function{data()}.
}
...
Chrom1l
@
\Caution[-1in]{The arithmetic operations in this \function{mutate()} command are being executed in SQL, not in \R, and the palette of allowable functions is much smaller. It is not possible, for example, to compute the logarithm of the length here using \function{log()}.  For that we must first collect the data into a real data frame.}
...
\Rindex{mystats()}%
\Rindex{function()}%
\marginnote{There are ways to check the \rterm{class} of an argument to see if it is a data frame, a vector, numeric, etc.  A really robust function should check to make sure that the values supplied to the arguments are of appropriate types.}%
...
file management techniques as in Figure ??.


\begin{center}
\includegraphics[width=3.5in]{images/dropbox1.png}
\end{center}
\marginnote{Figure ??. Dragging a csv file to a Dropbox Public directory}
...
\end{center}
\marginnote[2cm]{Getting the URL of a file in a Dropbox Public directory}
...
The same technique can be applied to all sorts of files like \R\ workspaces or \R\ scripts (files containing code). Of course, your students need to
use the appropriate \R\ command: \function{load()} for a workspace or \function{source()} for a script.
