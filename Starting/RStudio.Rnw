...
\subsection{Working with RMarkdown, Sweave and knitr}
...
\marginnote{RMarkdown, Sweave, and knitr files do not have access to the console workspace, 
so the code in them must be self-contained.}%
It is important remember that unlike \R\ scripts, which are executed in the
console and so do have access to the console workspace,  RMarkdown, Sweave, and knitr 
