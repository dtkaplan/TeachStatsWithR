...
\begin{itemize}
	...
	\item
		...
		workspace.
		\marginnote{\RStudio\ is like Facebook for statistics.}%
	\item
...
\marginnote[-2in]{We find it convenient to put the console in the upper left rather than the default 
location (lower right) so that students can see it better when we project or \R\ session in class.}
...
provides a brief description of the mark commands supported, and the \RStudio\ web site includes more
extensive tutorials on using RMarkdown.

\marginnote{RMarkdown, and \pkg{knitr}/\LaTeX\ files do not have access to the console workspace, 
so the code in them must be self-contained.}%
It is important remember that unlike \R\ scripts, which are executed in the
console and have access to the console workspace,  RMarkdown and \pkg{knitr}/\LaTeX\
files do not have access to the console workspace.  This is a good feature because it forces
...
The \tab{Workspace} tab shows the objects available to the console.  These are
