...
you will see something like this:  %Figure~\ref{fig:Rstudio-bigview}.
...
location (lower right) so that students can see it better when we project our \R\ 
session in class.}
...
so the code in them must be self-contained.  That is, the code should be complete enough to run even if the console had not been used at all.}
%
