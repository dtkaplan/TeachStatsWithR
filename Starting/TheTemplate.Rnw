...
\hfill -- Antoine de Saint-Exupery,  writer, poet, pioneering aviator
\end{quote}
...
\Rindex{lattice}
\Rindex{ggvis}
\Pointer[-1cm]{We are often asked about the other graphics systems, especially 
...
\TeachingTip{This plot can make an interesting discussion starter
...
\pkg{vcd} package provides a \function{mosaic()} function that creates mosaic plots.}
\Rindex{vcd}
\myindex{mosaic plot}
...
These and \function{xyplot()} or \function{plotPoints()} can also be used with one 
...
\Pointer{We could also use the \function{wday()} function
