...
\section{The \texttt{mosaic} package and the formula temaplate}
...
\TeachingTip{We recommend showing some plots on the first day and having student
...
\TeachingTip{This plot can make an interested discussion starter
...
@
		...
		the day of the year (\variable{dayofyear}) along the $x$-axis.
...
\InstructorNote{Introducing \option{width} and \option{center} here is perhaps
a violation of our usual policy of accepting defaults and saving options for later.
...
from the plot produced by \function{dotPlot()}, 
which is what students will more commonly think of as a dot plot.}
...
\Note{\texttt{\%\%} performs modular arithmetic, in this 
...
\end{center}
The
plots can be made using \pkg{lattice} or \pkg{ggplot2}, and there is a ``Show expression''
