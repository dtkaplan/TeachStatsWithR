...
\chapter[What Students Need]{What Students Need to Know About R\\ 
\& How to Teach It}
...
intuitive, while the other prefers the arrow operator because
...
\Caution{Not all package authors are equally skilled at creating examples.  
...
\marginnote{If you set up an \RStudio\ server, you can install all
...
\end{itemize}
\end{boxedText}
\marginpar{To help students keep variables and data frames straight, and to
...
We can gain access to a single variable in a data frame using the \verb!$! operator using
the syntax
...
dataframe$variable
@
But as we shall demonstrate, there are relatively few instances where one needs to do so.
For example,
...
The \verb!attach()! function in R can be used to make objects within data frames
...
variables using the \verb!$! syntax or to use functions that allow you to 
avoid the \verb!$! operator.
...
\verb!lattice! graphics.   Whenever the \pkg{mosaic} package is loaded, the \pkg{lattice}
...
\begin{itemize}
\item \verb!dotPlot! (notice the capital P)
...
\item \verb!histogram! or \verb!xhistogram!  (for histograms)
...
\Caution{Even when \texttt{y} is missing, the \texttt{\~} is still mandatory.}
\item
...
dotPlot(~ births, data=Births78, n=30)    
@
We can use a conditional variable to give us separate dot plots for each
species, and shrink the dots.
<<rs-dotPlot-condB,tidy=FALSE,cache=FALSE,out.width=".8\\textwidth",fig.width=7>>=
...
Histograms are a lot like dot plots, but the towers of dots are replaced by vertical bars.
Again the \verb!y! component of the formula is empty since we let \R\
compute the heights of the bars for us.
<<rs-iris-histogram,cache=TRUE,fig.width=8,out.width=".8\\textwidth">>=
...
<<rs-iris-histogram-cond,cache=TRUE,fig.width=8,out.width=".8\\textwidth">>=
...
<<rs-iris-bwplot-cond,cache=TRUE,fig.width=7,fig.height=2,out.width=".7\\textwidth">>=
bwplot(~ Sepal.Length | Species, data=iris, layout=c(3,1))
@
But there are better ways to do this.
\InstructorNote{This assumes that the variable on the right hand side of the \tilde\
is categorical}%
...
<<rs-iris-xyplot-cond,tidy=FALSE,cache=TRUE,fig.width=7,out.width=".7\\textwidth">>=
...
\marginnote{You can save all of this exporting and copying and pasting if you use RMardown or 
knitr/Sweave to prepare your documents.}
\begin{enumerate}
\item
...
@
The \verb!favstats()! function in the \pkg{mosaic} package computes several numerical 
...
\begin{itemize}
	\item
		Removes the need for the \verb!$! operator.
...
You can enter these tables manually as follows using a combination
...
already tabulated in this way.
<<>>=
barchart( mycrosstable )
@
\fi
...
root of 123.
\\[3mm]
\verb!x <- c(1,2,3)! & Make a vector containing values 1, 2, and 3 (in that order).
\\[3mm]
...
%	they fall into. 
%\\[3mm]
\texttt{xhistogram(\~{}Sepal.Length|Species, }

\hspace{.5in} \texttt{data=iris)}
&
...
\\[3mm]
\texttt{xyplot(Sepal.Length\~{}Sepal.Width | }

\hspace*{.5in}\texttt{Species, data=iris)} 
& 
Scatterplot of \verb!Sepal.Length! by \verb!Sepal.Width! 
with separate panels for each  \verb!Species!.
\\[3mm]
\texttt{tally(\~{} sector, CPS85)} & Frequency table of the variable \verb!sector!.
\\[3mm]
\texttt{bargraph(\~{}sector, CPS85))}
& Make a barchart from the table.
\\[3mm]
\texttt{tally(\~{}sector + race, CPS85)}
