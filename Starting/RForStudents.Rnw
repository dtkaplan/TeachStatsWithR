...
\chapter{What Students Need to Know About R 
\& How to Teach It}
...
<<rs-apropos>>=
apropos('table')            # must include quotes.  single or double.
@
...
<<rs-iris-Sepal-Length,eval=FALSE>>=
iris$Sepal.Length
@
\noindent
or 
...
\newthought{The RStudio interface} provides some GUI tools for loading data.
...
desktop machines will need to know how to install and load packages, however.}
...
<<rs-formula-interface, eval=FALSE, echo=TRUE>>=
