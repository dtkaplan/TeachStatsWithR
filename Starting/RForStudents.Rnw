...
\end{itemize}
\end{boxedText}
\marginnote{To help students keep variables and data frames straight, and to
...
\marginnote{The names in the \dataframe{faithful} data set are 
...
\marginnote{The interesting feature of this distribution is the clear bimodality.
...
<<>>=
freqpolygon( ~ Sepal.Length, data=iris, groups=Species, ylim=c(0,65))
@
...
Sweave or knitr to prepare your documents.}
\begin{enumerate}
\item
...
@
<<rs-bargraph2a>>=
bargraph(~ sector, data=CPS85, horizontal=TRUE)  # horizontal bars
@
...
in RMarkdown, Sweave, or knitr files.}
...
\section{\R\ Examples}
\vspace{-3mm}
The commands below are illustrated with the data sets \dataframe{iris} and 
\dataframe{CPS85}.  To apply these in other situations, you will need to 
substitute the name of your data frame and the variables in it.

\vspace{-3mm}
\begin{center}
\begin{longtable}{p{2.45in}p{3.30in}}
\verb!answer <- 42! & Store the value 42 in a variable named \variable{answer}.
\\[3mm]
