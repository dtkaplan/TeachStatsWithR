...
\item \function{histogram()} or \function{histogram()}  (for histograms)
	\footnote{\function{histogram()} is a \pkg{mosaic} function that adds some 
...
\medskip
\subsubsection{Histograms: \texttt{histogram()}}
