...
<<rs-iris-dotPlot,cache=FALSE,fig.width=5,fig.height=3.0>>=
...
<<rs-iris-dotPlot-condB,cache=FALSE,out.width=".8\\textwidth",fig.width=7>>=
...
<<rs-iris-xyplot-cond,cache=TRUE,fig.width=7,out.width=".7\\textwidth">>=
...
<<rs-iris-xyplot-groups,cache=TRUE>>=
...
The \pkg{mosaic} package provides a \term{formula interface} for each of these summaries.
...
of one categorical variable,  \pkg{mosaic} plots can do the same for cross tables.
The
...
\begin{problem}
The \dfn{Utilities2} data set in the \pkg{mosaic} package contains information
...
\label{sec:usingdata}
\R\ provides \function{read.csv()} (for comma separated values files),
...
can be exported as a csv file for subsequent reading in \R.  If you have python
installed on your system, you can also use \function{read.xls()} from the 
\pkg{gdata} package to read read directly from Excel files without this extra step.
...
In any case, students need to be trained to follow good data organization
practices:
\begin{itemize}
