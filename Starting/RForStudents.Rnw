...
\begin{boxedText}
	From this point on, we will assume that the \pkg{mosaic} and \pkg{mosaicData}
...
\Pointer{When typing a function name in the \RStudio\ console, pressing the tab key will produce a drop-down menu with a list of possible completions of the part of the function name you have already typed. Often, typing the first two or three characters of the function name is enough for this "tab-key completion" to be helpful. Similarly, after you have completed the function name and added the opening  parenthesis, pressing tab will show a concise list of the arguments to the function.  This is often enough of a reminder to get you going without using \code{?}.}
...
<<rs-apropos>>=
apropos('tally')    # search term must be quoted.  
@
...
\TeachingTip{Provide students with the commands for re-constructing a tabulation table when they are working with materials from a book that shows only the tabulation, not the case-by-case data behind it.}

Because categorical data is so easy to summarize in a table, 
books often give only the frequency or contingency tables.
