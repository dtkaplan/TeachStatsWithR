...
\marginnote[-6ex]{When introducing \R\ code to students, we emphasize the 
following questions:  What do you want \R\ to do for you?  What information must
you provide, if \R\ is going to do that?  The first question generally determines 
the function that will be used.  The second determines the inputs to that function.}%
