...
\R\ is a system for statistical computation and graphics, while
\RStudio\ is an integrated development environment that facilitates use of \R.  We use these systems for several reasons:
\begin{enumerate}
...
Once you have logged in to an \RStudio\ server, you will see something like this: % Figure~\ref{fig:Rstudio-bigview}.

IS THIS THE RIGHT PLACE?

%\begin{figure}
\begin{center}
\includegraphics[width=1.0\textwidth]{images/RStudio-bigview.png}
\end{center}
...
\TeachingTip{It's best to settle on using 
...
\authNote{Something funky going on with newproduct}
<<intro-variables1>>=
...
(see section \ref{sec:scripts}).
Comments are prefaced with the \# character.

<<intro-variables1b,eval=FALSE>>=
product                     # display the result
product <- 15.3 * 23.4      # <- is assignment operator, almost always the same as =
product                     
15.3 * 23.4 -> newproduct   # -> assigns to the right: be careful!
newproduct
@
...
<<intro-variables-semi,tidy=FALSE>>=
15.3 * 23.4 -> product; product    # save result and show it
@


\subsection*{Four Things to Know About \R\ and \RStudio}
\begin{enumerate}
...
<<"intro-function-syntax",eval=FALSE>>=
...
\tab{Packages} tab or by using \verb!require()! (or \verb!library()!).  This
...
% <<intro-help2,eval=FALSE,results="hide">>=
...
For example,
<<"intro-example-histogram",eval=FALSE,results="hide">>=
example(histogram)
@
...
<<"intro-demo",eval=FALSE,results="hide">>=
demo(lattice)
@
...
<<"intro-demo-list",eval=FALSE,results="hide">>=
demo()                     # all demos
...
\authNote{add info on Mosaic packages}
You can see a list of all data sets in all loaded packages
using 

<<"intro-data-list",eval=FALSE,results="hide">>=
data()
@
...
we'll describe how to get your own data into \R and \RStudio.  For now we'll use 
some data that is all ready for you to use.
...
<<"intro-view1",eval=FALSE>>=
View(iris)
@
to see the data or 
<<"intro-view2",eval=FALSE>>=
?iris
@
...
<<"intro-iris-Sepal-Length",eval=FALSE>>=
iris$Sepal.Length
@
or 
<<eval=FALSE>>=
...
%
<<intro-findData-human,eval=FALSE,echo=FALSE>>=
abdData('human')         # all data sets with 'human' in the name
@
%
<<intro-findData-2,eval=FALSE,echo=FALSE>>=
...
<<intro-formula-interface, eval=FALSE, echo=TRUE>>=
...
<<intro-iris-dotPlot,cache=TRUE,fig.width=5,fig.height=3.0>>=
...
<<intro-iris-dotPlot-condA,cache=TRUE,fig.width=7.0,fig.height=4,out.width=".7\\textwidth",eval=FALSE>>=
dotPlot(~ Sepal.Length | Species, data=iris, n=20, layout=c(3,1))
@
\authNote{Can these be set up to stack?}
<<intro-iris-dotPlot-condB,cache=FALSE,out.width=".8\\textwidth",fig.width=7>>=
...
<<intro-iris-histogram,cache=TRUE,fig.width=8,out.width=".8\\textwidth">>=
...
\authNote{removed plot that was breaking compilation}
%<<intro-iris-histogram-cond,cache=TRUE,fig.width=8,out.width=".8\textwidth">>=
%histogram(~ Sepal.Length | Species, data=iris, n=20, layout=c(3,1))
%@
...
<<intro-iris-bwplot-cond,cache=TRUE,fig.width=7,fig.height=2,out.width=".7\\textwidth">>=
...
<<"intro-iris-bwplot-2d",cache=TRUE>>=
...
<<intro-iris-xyplot-groups,cache=TRUE>>=
...
<<intro-iris-xyplot-key,cache=TRUE>>=
xyplot(Sepal.Length ~ Sepal.Width, groups=Species, data=iris, 
	auto.key=TRUE)   
@
...
<<intro-iris-xyplot-alpha,cache=TRUE>>=
xyplot(Sepal.Length ~ Sepal.Width, groups=Species, data=iris, 
...
<<intro-iris-xyplot-text,cache=TRUE>>=
xyplot(Sepal.Length ~ Sepal.Width, groups=Species, data=iris, 
	main="Some Iris Data",
...
<<intro-themes-whitbg,cache=TRUE,fig.height=4.0,fig.width=6>>=
trellis.par.set(theme=col.whitebg())      # a theme in the lattice package
show.settings()
@

<<intro-themes-mosaic,cache=TRUE,fig.height=4,fig.width=6>>=
...
between census years. These \dataframe{CPS85} data frame consist of a random sample of 
persons from the 1985 CPS, with information on wages and other characteristics of the workers,
...
\newthought{Calculating Frequency and Contingency Tables from Data}
Categorical variables are  often summarized in a table.  
\R\ can make a table for a categorical variable using \function{tally()}.

<<intro-table1>>=
tally(~ race, CPS85, margins=TRUE)
tally(~ sector, CPS85, margins=TRUE)
@
...
This is often a useful view of 
data with two categorical variables.


<<intro-xtabs>>=
tally(~ race & sector, CPS85, margins=TRUE)   # 2-way cross-table
@
...
\newthought{Entering Tables by Hand}
...
%\newthought{Bar charts and pie charts}
...
our plot.
<<intro-bargraph3a, tidy=FALSE, out.width=".47\\textwidth">>=
bargraph(~ sector, CPS85, groups=race, 
		 scales=list(x=list(rot=45)),
		 auto.key=list(space="right"))
bargraph(~ sector | race, CPS85, 
		 scales=list(x=list(rot=45)))
@
...
<<intro-mosaic1,fig.width=4,fig.height=2>>=
require(vcd)                         # load the visualizing categorical data package
mosaic(~ sex + union, CPS85)
@
\authNote{NH: this is feeling pretty rough here}
...
<<intro-mosaic3,fig.width=6,fig.height=2.5,eval=FALSE>>=
mosaic(mycrosstable)
@
...
\begin{problem}
The \dataframe{Utilities2} data set in the \verb!mosaic! package contains a number of variables
...
To do this in \R\, use \verb!factor(month)! in place of \dataframe{month}.  
...
your text as text.
<<intro-noquotes-error,error=TRUE>>=
text3 <- blah
@
...
<<"intro-function-syntax-template",eval=FALSE>>=
functionname( argument1, argument2, ... )
@
\vspace{-5mm}
\begin{itemize}
...
<<intro-argument-order-good,eval=FALSE>>=
...
<<intro-argument-order-bad,eval=FALSE>>=
...
We primarily use an online version of 
\Rstudio.  
\Rstudio\ is a innovative and powerful interface to \R\ that runs in a web browser 
or on your local machine.
Running in the browser has the advantage that you don't have to install or configure anything.  Just login
...
Your systems administrator will likely need to set up your own version of \RStudio\ for your institution, but we can attest that the process is straightforward and
greatly facilitates student and faculty use.
