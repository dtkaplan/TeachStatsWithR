...
on Teaching Statistics and revised for USCOTS 2011 and eCOTS 2014.  
...
We received great feedback and many wonderful ideas from the participants and those
...
and appropriate statistical analysis workflow.
This follows the
...
\subsection*{An Evolving Work}

The participants in our workshops often express a preference for these notes in printed rather than electronic form.  We have therefore made a small book containing the notes (which may well be in your hands as you read this).  Despite the ergonomic advantages of the printed form, it's impossible to update and expensive to distribute.

To take advantage of electronic forms, we are making these notes available in electronic forms.

\begin{enumerate}
\item As a PDF file with the same pagination as the book.  Fans of the fantastic graphics books by Edward Tufte may note that we use a page format reminiscent of Tufte's.  We find this works well both on the printed page and on a tablet computer.
\item As text files suitable for compilation with \LaTeX\~and \code{knitr} into the print-ready form.  
\end{enumerate}

Both of these are available via GitHub at \url{https://github.com/dtkaplan/TeachStatsWithR}.
...
Software is always a work in progress, and so must be any notes based on software.
...
The primary audience for these materials is instructors of statistics at the college or university level.  A secondary audience is the students these instructors teach. 
...
both novice and expert users.  We have adopted it as our standard teaching environment
...
that is accessible to users via the Internet.
...
For instructors, as well as their students, we recommend writing with \pkg{knitr} with RMarkdown.  These tools have very little start-up cost and provide a way to write notes and reports with narrative text and traditional mathematical notation integrated with R commands. This works well even with beginning students, but also handles the needs of upper-level students, instructors, and researchers.

\DiggingDeeper{The \pkg{knitr} package can also be used with the professional-level \LaTeX\ typesetting system. 
We used this system to produce this book.  }
...
\term{data science}, defined as the ability to use data to answer questions
...
on this book, we developed and update the \pkg{mosaic}
...
sets, as well as answers to questions that we have heard frequently from both students and faculty colleagues.  We invite you to scavenge from our materials and ideas  and modify them to fit your courses and your students.
