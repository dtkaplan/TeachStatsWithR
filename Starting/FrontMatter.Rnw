...
opts_chunk$set( fig.path="figure/FrontMatter-fig-" ) 
standalone=TRUE
...
We organized the workshop to help instructors integrate \R\ (as well as some 
...
that we've shared this with since the workshop.  
...
\RStudio\ is an integrated development environment that facilitates use of \R\ for
novice and expert users.  We have adopted it as our standard teaching environment
because it simplifies the use of \R\ for instructors and faculty.
...
will be used throughout.  Other packages appear from time to time, including
\begin{multicols}{3}
\begin{itemize}
\item
\pkg{fastR}
...
\today, using \code{knitr} and 
\Sexpr{print(version$version.string)} 
\DiggingDeeper{If you know \LaTeX\ as well as \R, then \code{knitr}
provide a nice solution for mixing the two. We tend to use these systems
for reproducible analysis
in our upper level courses, as well as for our other work. }
...
The focus of the project is to highlight and integrate
...
such as USCOTS 2011 workshop on \emph{Teaching Statistics Using \R\ and \RStudio}, our 2010 
...
\chapter*{Statistical Computation and Computational Statistics}
...
shaping how statistics is done and taught.
...
\authNote{Should we say something about the current discussion in stat ed about
randomization tests and bootstrap?}%

Among the various statistical software packages available, \R\ is becoming
increasingly popular.  Because \R\ and \RStudio\ are free, they
have become widely
...
An outline of the book could go here once we decide what the outline is.
\authNote{Need to add outline here}
