...
<<manipulate-histogram3, eval=inRStudio>>=
manipulate(
...
<<manipulate-xpnorm, tidy=FALSE, eval=inRStudio>>=
  manipulate( xpnorm( X, 500, 100, verbose=FALSE, invisible=TRUE ), 
              X = slider(200,800) )
@
...
<<manipulate-xpnorm2, eval=inRStudio>>=
  manipulate( xpnorm( c(-X,X), 500, 100, verbose=FALSE, invisible=TRUE ), 
              X = slider(200,800) )
@
...
mNorm <- function( mean=0, sd=1 ) {
  lo <- mean - 5*sd 
  hi <- mean + 5*sd
  manipulate( xpnorm( c(A,B), mean, sd, verbose=FALSE, invisible=TRUE ), 
